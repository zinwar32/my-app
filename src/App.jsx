...
function ProjectInsightsTab({ project, data, setData, addToast }) {
  ...
  const handlePrint = () => {
    try {
      ...
      if (!reportWindow) {
        addToast("Please allow popups for this site to print reports", "error");
        return;
      }
      ...
      // Build HTML in parts to avoid template issues
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body {
            ...
            line-height: 1.6; color: #1a1a1a; background: white; font-size: 12px;
        }

        .report-header {
            background: linear-gradient(135deg, #0A0C14 0%, #1F2937 100%);
            color: white; padding: 40px 30px; margin-bottom: 30px;
        }

        .report-title { font-size: 32px; font-weight: 700; margin-bottom: 10px; }
        .report-meta { display: flex; gap: 30px; font-size: 14px; opacity: 0.9; }

        .kpi-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 20px; margin-bottom: 30px; }
        .kpi-card { background: white; border: 2px solid #e5e7eb; border-radius: 12px; padding: 20px; text-align: center; }
        .kpi-value { font-size: 28px; font-weight: 700; color: #3B82F6; margin-bottom: 5px; }
        .kpi-label { font-size: 12px; color: #6b7280; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px; }

        .section { margin-bottom: 40px; page-break-inside: avoid; }
        .section-header { background: #f8fafc; border-left: 4px solid #3B82F6; padding: 15px 20px; margin-bottom: 20px; }
        .section-title { font-size: 18px; font-weight: 600; color: #1f2937; margin-bottom: 5px; }
        ...
        .data-item { background: #f8fafc; padding: 12px 15px; border-radius: 8px; border: 1px solid #e5e7eb; }
        .data-label { font-size: 10px; color: #6b7280; text-transform: uppercase; font-weight: 600; letter-spacing: 0.5px; margin-bottom: 4px; }
        .data-value { font-size: 13px; color: #1f2937; font-weight: 500; }

        .table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        .table th, .table td { padding: 8px 12px; text-align: left; border-bottom: 1px solid #e5e7eb; }
        .table th { background: #f8fafc; font-weight: 600; font-size: 11px; color: #374151; text-transform: uppercase; letter-spacing: 0.5px; }
        .table td { font-size: 12px; color: #4b5563; }
        ...
        .insights-section { background: #f8fafc; border-radius: 12px; padding: 20px; margin-top: 20px; }
        .insight-item { margin-bottom: 15px; }
        .insight-title { font-weight: 600; color: #1f2937; margin-bottom: 5px; font-size: 14px; }
        .insight-content { color: #4b5563; line-height: 1.6; }

        .action-items { margin-top: 15px; counter-reset: action-counter; }
        .action-item { display: flex; align-items: flex-start; margin-bottom: 8px; padding-left: 20px; position: relative; }
        .action-item:before { content: counter(action-counter); counter-increment: action-counter; position: absolute; left: 0; top: 0; background: #3B82F6; color: white; width: 16px; height: 16px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 600; }

        @page { margin: 0.5in; size: A4; }
        @media print { body { font-size: 11px; } .report-title { font-size: 28px; } }
    </style>
</head>
<body>
    ...
    <div class="section">
        ...
        <div class="kpi-grid">
            <div class="kpi-card"><div class="kpi-value">${kpis?.overall?.toFixed(1) || 'N/A'}</div><div class="kpi-label">Overall Score</div></div>
            <div class="kpi-card"><div class="kpi-value">${kpis?.partSatAvg?.toFixed(1) || 'N/A'}</div><div class="kpi-label">Participant Satisfaction</div></div>
            <div class="kpi-card"><div class="kpi-value">${kpis?.stakeSatAvg?.toFixed(1) || 'N/A'}</div><div class="kpi-label">Stakeholder Satisfaction</div></div>
            <div class="kpi-card"><div class="kpi-value">${kpis?.nps?.toFixed(1) || 'N/A'}</div><div class="kpi-label">Net Promoter Score</div></div>
            <div class="kpi-card"><div class="kpi-value">${kpis?.taskProgress || 0}%</div><div class="kpi-label">Task Completion</div></div>
            <div class="kpi-card"><div class="kpi-value">${kpis?.budgetVar ? (kpis.budgetVar * 100).toFixed(1) + '%' : 'N/A'}</div><div class="kpi-label">Budget Variance</div></div>
        </div>
    </div>`;
    ...
    <div class="section">
        ...
        <div class="data-grid">
            <div class="data-item"><div class="data-label">Location</div><div class="data-value">${project.location || 'Not specified'}</div></div>
            <div class="data-item"><div class="data-label">Target Audience</div><div class="data-value">${project.target_audience || 'Not specified'}</div></div>
            <div class="data-item"><div class="data-label">Planned Budget</div><div class="data-value">${project.planned_budget ? project.planned_budget.toLocaleString() + ' SAR' : 'Not set'}</div></div>
            <div class="data-item"><div class="data-label">Actual Budget</div><div class="data-value">${project.actual_budget ? project.actual_budget.toLocaleString() + ' SAR' : 'Not set'}</div></div>
            <div class="data-item"><div class="data-label">Planned Attendance</div><div class="data-value">${project.planned_attendance || 'Not set'}</div></div>
            <div class="data-item"><div class="data-label">Actual Attendance</div><div class="data-value">${project.actual_attendance || 'Not set'}</div></div>
            <div class="data-item"><div class="data-label">Start Date</div><div class="data-value">${project.start_date || 'Not set'}</div></div>
            <div class="data-item"><div class="data-label">End Date</div><div class="data-value">${project.end_date || 'Not set'}</div></div>`;

      if (project.objective) {
        reportHTML += `<div class="data-item" style="grid-column: 1 / -1;"><div class="data-label">Objective</div><div class="data-value">${project.objective}</div></div>`;
      }
      if (project.success_metrics) {
        reportHTML += `<div class="data-item" style="grid-column: 1 / -1;"><div class="data-label">Success Metrics</div><div class="data-value">${project.success_metrics}</div></div>`;
      }
      reportHTML += `</div></div>`;
    ...
    <div class="section">
        ...
        <table class="table">
            <thead><tr><th>Name</th><th>Role in Project</th><th>Responsibility</th><th>Performance</th></tr></thead>
            <tbody>`;
        teamMembers.forEach(member => {
          reportHTML += `<tr><td>${member.memberName}</td><td>${member.role_in_project || 'Not specified'}</td><td>${member.responsibility || 'Not specified'}</td><td>${member.performance_score ? (member.performance_score * 20) + '/100' : 'Not rated'}</td></tr>`;
        });
    ...
    <div class="section">
        ...
        <div style="margin-bottom: 15px;"><strong>Completion Rate:</strong> ${Math.round((completedTasks / tasks.length) * 100)}% (${completedTasks}/${tasks.length} tasks completed)</div>
        <table class="table">
            <thead><tr><th>Task</th><th>Phase</th><th>Priority</th><th>Status</th><th>Due Date</th><th>Owner</th></tr></thead>
            <tbody>`;
        tasks.forEach(task => {
          const owner = data.teamMembers.find(m => m.id === task.owner_member_id);
          reportHTML += `<tr><td>${task.task_title}</td><td>${task.phase || 'Not set'}</td><td><span class="status-badge priority-${task.priority?.toLowerCase() || 'medium'}">${task.priority || 'Medium'}</span></td><td><span class="status-badge status-${task.status?.toLowerCase().replace(' ', '') || 'todo'}">${task.status || 'To Do'}</span></td><td>${task.due_date || 'Not set'}</td><td>${owner?.name || 'Unassigned'}</td></tr>`;
        });
        reportHTML += `</tbody></table></div>`;
      }

      // Feedback
      if (feedbackParticipants.length > 0 || feedbackStakeholders.length > 0) {
        reportHTML += `
    <div class="section">
        <div class="section-header">
        ...
        <div class="data-grid">
            <div class="data-item"><div class="data-label">Participant Responses</div><div class="data-value">${feedbackParticipants.length}</div></div>
            <div class="data-item"><div class="data-label">Avg Participant Satisfaction</div><div class="data-value">${avgParticipantSat}/5.00</div></div>
            <div class="data-item"><div class="data-label">Stakeholder Responses</div><div class="data-value">${feedbackStakeholders.length}</div></div>
            <div class="data-item"><div class="data-label">Avg Stakeholder Satisfaction</div><div class="data-value">${avgStakeholderSat}/5.00</div></div>
        </div>`;

        if (feedbackParticipants.length > 0) {
          reportHTML += `<div style="margin-top: 20px;"><h4 style="margin-bottom: 10px; color: #1f2937;">Recent Participant Feedback</h4><table class="table"><thead><tr><th>Date</th><th>Overall Rating</th><th>Best Part</th><th>Improvements</th></tr></thead><tbody>`;
          feedbackParticipants.slice(0, 5).forEach(f => {
            reportHTML += `<tr><td>${f.response_date}</td><td>${f.satisfaction_overall}/5</td><td>${f.best_part || '—'}</td><td>${f.improve || '—'}</td></tr>`;
          });
          reportHTML += `</tbody></table></div>`;
        }
        reportHTML += `</div>`;
    ...
    <div class="section">
        ...
        <table class="table">
            <thead><tr><th>Date</th><th>Objective Alignment</th><th>Execution Quality</th><th>Team Coordination</th><th>Risk Management</th><th>Went Well</th><th>Controllable</th></tr></thead>
            <tbody>`;
        internalReviews.forEach(review => {
          reportHTML += `<tr><td>${review.review_date}</td><td>${review.objective_alignment}/5</td><td>${review.execution_quality}/5</td><td>${review.team_coordination}/5</td><td>${review.risk_management}/5</td><td>${review.went_well || '—'}</td><td>${review.controllable || '—'}</td></tr>`;
        });
        reportHTML += `</tbody></table></div>`;
      }

      // Insights
      if (insights.strengths || insights.weaknesses || insights.action_items || insights.generated_summary) {
        reportHTML += `
    <div class="section">
        <div class="section-header">
        ...
        if (insights.generated_summary) {
          reportHTML += `<div class="insight-item"><div class="insight-title">AI Summary</div><div class="insight-content">${insights.generated_summary}</div></div>`;
        }
        if (insights.strengths) {
          reportHTML += `<div class="insight-item"><div class="insight-title">✨ Strengths</div><div class="insight-content">${insights.strengths}</div></div>`;
        }
        if (insights.weaknesses) {
          reportHTML += `<div class="insight-item"><div class="insight-title">⚠️ Areas for Improvement</div><div class="insight-content">${insights.weaknesses}</div></div>`;
        }
        if (insights.action_items) {
          reportHTML += `<div class="action-items"><div class="insight-title">🎯 Action Items</div>`;
          insights.action_items.split('\n').filter(item => item.trim()).forEach(item => {
            reportHTML += `<div class="action-item">${item.trim()}</div>`;
          });
          reportHTML += `</div>`;
        }
        reportHTML += `</div></div>`;
